...
export default () => {
  ...
  if (process.env.DATABASE_URL) {
    try {
      ...
      dbConfig.type = protocol;
      dbConfig.database = parsedUrl.pathname ? decodeURIComponent(parsedUrl.pathname.substring(1)) : dbConfig.database;
      dbConfig.host = parsedUrl.hostname;
      dbConfig.port = parsedUrl.port ? parseInt(parsedUrl.port, 10) : dbConfig.port;
      dbConfig.username = parsedUrl.username ? decodeURIComponent(parsedUrl.username) : dbConfig.username;
      dbConfig.password = parsedUrl.password ? decodeURIComponent(parsedUrl.password) : dbConfig.password;
    } catch (err) {
      console.error('[Bootstrap] Failed to parse DATABASE_URL:', err instanceof Error ? err.message : String(err));
    }
  }
